...
impl App {
    ...
    /// A styled strip standing in for the title bar's usual blank
    /// background, macOS only (see `main.rs`): there, `platform_specific`
    /// makes the real titlebar transparent and lets our content run up
    ...
    fn title_bar(&self) -> Element<'_, Message> {
        // Left-padded clear of the traffic lights, which this bar renders
        // behind rather than beside.
        let bar = container(text(self.title()).size(design::TEXT_SM))
        ...
        column![bar, rule::horizontal(1.0)].into()
    }
