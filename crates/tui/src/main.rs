...
fn run(terminal: &mut Tui) -> Result<Option<String>> {
    ...
    while !app.should_quit {
        ...
        match event::next_action(Duration::from_millis(250), app.mode(), app.active_tab)? {
            ...
            event::Action::OpenSettingsEdit => app.open_settings_edit(),
            event::Action::DiscoveryScan => app.run_discovery(),
            event::Action::DiscoveryTrackAll => app.discovery_track_all()?,
            event::Action::DiscoveryActivate => app.discovery_activate_selected()?,
